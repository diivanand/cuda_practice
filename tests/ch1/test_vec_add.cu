...
TEST(VecAdd, SmallKnownInput) {
    ...
    ch1::vec_add<<<1, 32>>>(da.data(), db.data(), dout.data(), static_cast<int>(n));
...
TEST(VecAdd, AllZeros) {
    ...
    ch1::vec_add<<<1, 256>>>(da.data(), db.data(), dout.data(), static_cast<int>(n));
    CUDA_TRY(cudaGetLastError());
    CUDA_TRY(cudaDeviceSynchronize());
...
TEST(DeviceBuffer, AllocateSizeAndBytes) {
    ...
    const cuda_utils::device_buffer<float> buf(n);
